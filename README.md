...
- ファイル数・行数・文字数を集計して表示（3桁カンマ区切り・単位付き）
- 集計結果をワンクリックでクリップボードへコピー
- ファイルごとの一覧表示（行数・文字数）
- 対象拡張子や除外ディレクトリを設定可能
...
集計結果エリアは「コピー」ボタンでクリップボードへコピーできます
（3桁カンマ区切り・単位付きのテキスト形式）。
...
初回起動時に `user` フォルダが自動作成され、その中に `user/settings.yml` が生成されます。
...
| `last_folder` | `""` | 前回の集計フォルダ（次回起動時に自動復元） |
| `result_template` | 既定テンプレート | 集計結果の表示・コピー形式 |

GUI の「設定...」からも変更できます。

## 集計結果テンプレート

「設定...」で集計結果の表示・コピー形式をカスタマイズできます。
以下のプレースホルダが利用可能です。

| プレースホルダ | 内容 |
| --- | --- |
| `{path}` | 集計対象の絶対パス |
| `{files}` | ファイル数（3桁カンマ区切り・単位付き） |
| `{lines}` | 行数（3桁カンマ区切り・単位付き） |
| `{chars}` | 文字数（3桁カンマ区切り・単位付き） |
...
📊 集計 ({date})
パス: {path}
ファイル: {files} / 行: {lines} / 文字: {chars}
```

## 前回フォルダの復元

- 集計したフォルダは `last_folder` に自動保存されます
- 次回起動時にそのフォルダが選択された状態で開きます
- 保存したフォルダが存在しなくなった場合は、exe のあるフォルダ（
  ソース実行時はカレントディレクトリ）が初期表示されます
